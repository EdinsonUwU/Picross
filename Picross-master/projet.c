...
void ncurses_initialiser(){  //Initialisation of ncurses (given)
  ...
  cbreak();	            /* For keyboard input (disable buffering) */
  noecho();             /* Deactivate the display of typed characters */
  keypad(stdscr, TRUE);	/* Activate the specific keys*/
...
void ncurses_couleurs(){  //Initialisation of the colours (given)
  ...
  if(has_colors() == FALSE) {
    endwin();
    fprintf(stderr, "The terminal do not support the colours.\n");
    exit(EXIT_FAILURE);
  }
...
void ncurses_souris(){  //Initialisation of the mouse (given)
  if(!mousemask(ALL_MOUSE_EVENTS, NULL)) {
    endwin();
    fprintf(stderr, "error initializing mouse.\n");
    exit(EXIT_FAILURE);
  }
